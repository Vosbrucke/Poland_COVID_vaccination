...
df <- read.csv("Raw_data/wyniki_gl_na_listy_po_powiatach_proc_sejm.csv", sep = ";", dec = ",")[, c(1:4, 10, 12, 13, 15, 17)] %>% 
  set_colnames(c("Kod.TERYT", "Powiat", "Województwo", "Frekwencja", "KOMITET_KO", "KOMITET_Konfederacja", "KOMITET_PSL", "KOMITET_PiS", "KOMITET_SLD"))
...
df %<>%
  mutate(
    ...
    Kod.TERYT = str_sub(Kod.TERYT, 1, 5),
...
# Save the plot
ggsave("Plots/Winning_party.png", dpi = 900, height = 16, width = 25, units = "cm")
